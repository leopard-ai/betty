...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
		   ...
		   'wide_resnet50_2', 'wide_resnet101_2', 'RNNDecoder', 'Learner']
	...
	def forward(self,logits, targets):
		try:
			...
			for i in range(len(targets)):
				ones[i,targets_int[i]] = targets[i] - targets[i].detach() + 1.
		except:
			print("error with targets: ",targets)
		return torch.sum(- ones.cuda() @ F.log_softmax(logits+self.eps, -1).T, -1).mean().cuda()		
...
class RNNDecoder(nn.Module):
	...
	def __init__(self, vocab_size, hidden_size=1024, embed_size=1024, num_layers=1, max_seq_length=16):
		...
		self.cls = nn.Sequential(nn.Linear(hidden_size, 20000),
								nn.Dropout(p=0.3),
								nn.ReLU(),
	...
	def forward(self, features, captions, lengths):
		...
		start_tokens = torch.ones(batch_size).cuda() * self.vocab_size   # 2) pass <START> token
		...
		embeds = torch.cat((features.unsqueeze(1), embeds), 1) # append features to embeddings
		# print("embed, length shape :",embeds.shape, len(lengths))
		packed = pack_padded_sequence(embeds, lengths, batch_first=True) #pack the input to skip padding backprop
	...
	def sample(self, features, states=None):
		...
		for i in range(self.max_seq_length):
			...
			inputs = self.embed(predicted.detach().clone())                       # inputs: (batch_size, embed_size)
...
class Learner(nn.Module):
	...
	def __init__(self, enc_arch, decoder, vocab_size):
		...
		self.lin = nn.Linear(512 * self.enc.block.expansion, self.dec.hidden_size).cuda()
		# if is_cifar100:
